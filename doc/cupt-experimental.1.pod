...
boolean, specifies whether should downloader follow HTTP redirects.
True by default.
...
integer, the "quality bar" for the native resolver. For the situations
...
list, source package names (Perl regular expressions) of known exceptions.
...
list, package names which will be upgraded indirectly
...
integer, positive, specifies how many times will worker try to generate unrelated
changesets if
