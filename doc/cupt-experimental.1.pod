...
=item cupt::resolver::tune-score::<part>

The group of integer options which control internal resolver's score
calculation. Values are relative, not absolute.
...
a multiplier for the L<cupt::resolver::quality-bar|/cupt::resolver::quality-bar>

=item failed-recommends

some 'recommends' dependency is not satisfied (when requested)

=item failed-suggests
