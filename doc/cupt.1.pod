...
work only with installed versions of packages; valuable only for L</show>, L</depends>, L</rdepends>, L</search>, L</pkgnames> actions
...
This subcommand receives the list of relations. Relation has the same syntax as
defined in Debian Policy, 7.1.
...
=item --install-unpacked

implicitly install all unpacked packages, useful for continuing interrupted
upgrades
...
Firstly, Cupt recognizes and uses following APT configuration variables:
...
apt::neverautoremove, apt::get::allowunauthenticated, dir, dir::bin::dpkg,
dir::cache, dir::cache::archives, dir::etc, dir::etc::sourcelist,
dir::etc::sourceparts, dir::etc::parts, dir::etc::main, dir::etc::preferences,
dir::state, dir::state::extendedstates, dir::state::lists, dir::state::status,
dpkg::pre-install-pkgs, gpgv::trustedkeyring

See L<apt.conf(5)> for their meanings.

Secondly, Cupt introduces following cupt-specific configuration variables:
...
boolean (0/1), specifies whether should resolver try to remove automatically
installed packages after doing requested actions or not. True by default.
