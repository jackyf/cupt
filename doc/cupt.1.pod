...
One caveat exists, though: --installed-only option won't work under 'shell'
subcommand.
...
unattended-upgrade::*, acquire::compressiontypes, apt::get::show-upgraded,
...
positive number, defines the priority of download protocol I<protocol>, determines an order in which
...
list of names of the methods available to download protocol I<protocol>
...
=item cupt::resolver::quality-bar

integer number, the "quality bar" for the native resolver. For the situations
when a lot of solutions available, increasing this value slows down choosing the
solution to propose, but may increase its quality. Defaults to -400. The sane
value borders: from -1000 to 1000.

=item cupt::resolver::external-command

a preview-only, experimental option to use external resolver using CUDF
...
=item cupt::resolver::synchronize-source-versions
...
It's highly recommended to have source entries for as more as possible
available binary packages for more fast and accurate results.
...
=item cupt::resolver::synchronize-source-versions::exceptions
...
=item cupt::worker::allow-indirect-upgrade

list of package names which will be upgraded indirectly

There are some packages with dependencies that make unable to upgrade the
package properly. List them here. See also Debian BTS #551831.

=item cupt::worker::archives-space-limit

integer, bytes, if set, limits the worker to not download more than specified
amount for archives, and use download-install-clean algorithm (download
...
=item cupt::worker::archives-space-limit::tries

integer, specifies how many times will worker try to generate unrelated
changesets if
L<cupt::worker::archives-space-limit|/cupt::worker::archives-space-limit> is
set. As genearting algorithm uses randomization, increasing this option
increases the changes to generate. 20 by default.
