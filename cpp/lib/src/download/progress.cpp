...
void Progress::progress(const vector< string >& params)
{
	...
	if (params.size() < 2)
	{
		fatal2(__("download progress: received progress message with less than 2 parameters"));
	}
	...
	else if (action == "start")
	{
		if (params.size() > 3)
		{
			fatal2(__("download progress: received progress submessage 'start' with more than 1 parameters"));
		}
		// new download
	...
	else
	{
		...
		{
			fatal2(__("download progress: received info for not started download, uri '%s'"), uri);
		}
		...
		if (action == "downloading")
		{
			if (params.size() != 4)
			{
				fatal2(__("download progress: received submessage 'downloading' with more than 2 parameters"));
			}
		...
		else if (action == "expected-size")
		{
			if (params.size() != 3)
			{
				fatal2(__("download progress: received submessage 'expected-size' with more than 1 parameter"));
			}
			record.size = lexical_cast< size_t >(params[2]);
			updateHook(true);
		}
		else if (action == "pre-done")
		{
			if (params.size() != 2)
			{
				fatal2(__("download progress: received submessage 'pre-done' with more than 0 parameters"));
			}
		...
		else if (action == "done")
		{
			if (params.size() != 3)
			{
				fatal2(__("download progress: received submessage 'done' with more than 1 parameter"));
			}
		...
		{
			fatal2(__("download progress: wrong action '%s' received"), action);
		}
	}
}
