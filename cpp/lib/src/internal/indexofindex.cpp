...
void parseFullIndex(const string& path, const Callbacks& callbacks, const Record& record)
{
	...
	while (true)
	{
		...
		*(record.offsetPtr) = offset;
		...
		{
			callbacks.main(buf + packageAnchorLength, size - packageAnchorLength - 1);
		}
		else
		{
...
void parseIndexOfIndex(const string& path, const Callbacks& callbacks, const Record& record)
{
	...
	while (file.rawGetLine(buf, bufSize), bufSize > 0)
	{
		{ // offset and package name:
			...
			auto packageNameBegin = (const char*)delimiterPosition+1;
			auto packageNameEnd = buf+bufSize-1;
			callbacks.main(packageNameBegin, packageNameEnd - packageNameBegin);
		}
...
void generate(const string& indexPath, const string& temporaryPath)
{
	...
	callbacks.main =
			...
			(const char* packageNameBuffer, size_t packageNameLength)
			{
				...
				file.put("\0", 1);
				file.put(packageNameBuffer, packageNameLength);
				file.put("\n");
	...
	parseFullIndex(indexPath, callbacks, { &offset });
