		...
		{ \
			...
			{ \
				warn2("package %s, version %s: unrecognized priority value '%s', using 'extra' instead", \
						v->packageId.name(), v->versionString, tagValue.toString()); \
			} \
		})
		...
		while (parser.parseNextLine(tagName, tagValue))
		{
			...
			if (Version::parseRelations)
			{
				...
				TAG(Provides,
				{
					...
					{
						v->provides.push_back(PackageId(begin, end-begin));
					};
					parse::processSpaceCharSpaceDelimitedStrings(tagValue.first, tagValue.second, ',', callback);
				})
			}
